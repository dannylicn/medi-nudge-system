...
## Purpose
Define the AWS service requirements for running the Medi-Nudge system in production. Covers compute (ECS Fargate), database (RDS PostgreSQL), object storage (S3), networking (VPC, ALB), CDN (CloudFront), secret management (Secrets Manager), and observability (CloudWatch).
