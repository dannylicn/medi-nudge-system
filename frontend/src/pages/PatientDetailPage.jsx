...
export default function PatientDetailPage() {
  ...
  const saveConditions = async () => { setSavingConditions(true); try { await updatePatient(id, { conditions: selectedConditions }); setEditingConditions(false); await reload(); } catch {} finally { setSavingConditions(false); } };
  ...
  const saveCaregiver = async () => { setSavingCaregiver(true); try { await updatePatient(id, caregiverForm); setEditingCaregiver(false); await reload(); } catch {} finally { setSavingCaregiver(false); } };
  const handleGenerateCaregiverLink = async () => { setCaregiverLinkLoading(true); try { const res = await generateCaregiverInviteLink(id); setCaregiverInviteLink(res.data.invite_link); } catch {} finally { setCaregiverLinkLoading(false); } };
  ...
  const handleRegenerateQR = async () => { setQrLoading(true); try { const res = await regenerateInviteLink(id); setQrCode(res.data.onboarding_qr_code); setInviteLink(res.data.invite_link); } catch {} finally { setQrLoading(false); } };
  ...
  const openAssignMed = async () => { try { const { data } = await getMedications(); setCatalogMeds([...data].sort((a, b) => { const aS = suggestedMedIds.has(a.id) ? 0 : 1; const bS = suggestedMedIds.has(b.id) ? 0 : 1; return aS !== bS ? aS - bS : a.name.localeCompare(b.name); })); } catch {} setAssignForm({ medication_id: "", dosage: "", refill_interval_days: "", frequency: "once_daily", reminder_times: "" }); setShowAssignMed(true); };
  const handleAssignMed = async (e) => { e.preventDefault(); setAssigningSaving(true); try { await assignMedication(id, { medication_id: Number(assignForm.medication_id), dosage: assignForm.dosage || null, refill_interval_days: assignForm.refill_interval_days ? Number(assignForm.refill_interval_days) : null, frequency: assignForm.frequency, reminder_times: assignForm.reminder_times ? assignForm.reminder_times.split(",").map((t) => t.trim()).filter(Boolean) : null }); setShowAssignMed(false); await reload(); } catch {} finally { setAssigningSaving(false); } };

  const openDispensing = async () => { try { const { data } = await getMedications(); setCatalogMeds(data); } catch {} setDispensingForm({ medication_id: "", dispensed_at: new Date().toISOString().slice(0, 16), days_supply: 30, quantity: "" }); setShowDispensing(true); };
  const handleDispensing = async (e) => { e.preventDefault(); setDispensingSaving(true); try { await createDispensingRecord({ patient_id: Number(id), medication_id: Number(dispensingForm.medication_id), dispensed_at: new Date(dispensingForm.dispensed_at).toISOString(), days_supply: Number(dispensingForm.days_supply), quantity: dispensingForm.quantity ? Number(dispensingForm.quantity) : null, source: "manual" }); setShowDispensing(false); await reload(); } catch {} finally { setDispensingSaving(false); } };
